...
const THEMES = [
	...
	["chalk.min.css,#151515,#d0d0d0", "Chalk", "icon color_lenspalette", true],
	["darcula.min.css,#2b2b2b,#a9b7c6", "Darcula", "icon color_lenspalette", true],
	[
	...
	[
		"github-dark.min.css,#0D1117,#0D1117",
		"Github Dark",
		"icon color_lenspalette",
		true,
	],
	["monokai.min.css,#272822,#f8f8f2", "Monokai", "icon color_lenspalette", true],
	["nord.min.css,#2e3440,#d8dee9", "Nord", "icon color_lenspalette", true],
	[
	...
	[
		"night-owl.min.css,#111423,##11162e",
		"night owl",
		"icon color_lenspalette",
		true,
	],
...
class CodeSnap {
	async init($page) {
		...
		document.head.append(this.$style, this.$githubDarkFile, this.$higlightJsFile);
	...
	createSnap() {
		...
		/* delete the looping that is here */
		let htmlSnippet = `<div id="snippet-scroll">
          ...
          <div id="window">
            <div id="navbar">
              ...
              <div id="window-title" contenteditable="true">${
															editorManager.activeFile.filename
														}</div>
            </div>
            <div id="snippet">
            <pre class="hljs codesArea"><code style="font-family: ${settings.get(
													"editorFont",
												)};">${hljs.highlightAuto(selectedText).value}</code></pre>   
            </div>
          </div>
         </div>
      </div>
